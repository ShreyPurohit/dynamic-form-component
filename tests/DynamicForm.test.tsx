...
describe("Dynamic Form Component", () => {
  ...
  test("renders fields and handles validation correctly", async () => {
    const inputs = [
      ...
      },
      {
        placeholder: "Enter your email",
        value: "invalid-email",
        errorMsg: /enter a valid email address/i,
      },
      {
        placeholder: "Enter your password",
        value: "123",
        errorMsg: /password must be at least 6 characters/i,
      },
      {
        placeholder: "Enter your age",
        value: "16",
        errorMsg: /you must be at least 18 years old/i,
      },
    ];
    ...
    for (const placeholder of requiredFields) {
      ...
      expect(
        await screen.findByText(
          new RegExp(`${placeholder.split(" ")[2]} is required`, "i"),
        ),
      ).toBeInTheDocument();
    }
  });
  ...
  test("checkbox group shows error when no checkbox is selected", async () => {
    fireEvent.click(screen.getByText("Submit"));
    expect(
      await screen.findByText(/at least one hobby must be selected/i),
    ).toBeInTheDocument();
  });
  ...
  test("renders null for unsupported field type", () => {
    ...
    expect(
      screen.queryByLabelText("Unsupported Field"),
    ).not.toBeInTheDocument();
  });
  ...
  test("button is disabled when specified", async () => {
    const disabledButton: ISubmitButtonProps = {
      ...submitButton,
      disabled: true,
